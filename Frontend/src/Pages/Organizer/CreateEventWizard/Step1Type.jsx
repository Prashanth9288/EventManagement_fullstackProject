...
const EventTypeCard = ({ type, title, description, icon, selected, onClick }) => (
  ...
  >
    <div className={`text-4xl ${selected ? 'text-indigo-600' : 'text-gray-400'}`}>
      {icon}
    </div>
    <div>
      <h3 className={`font-semibold text-lg ${selected ? 'text-indigo-900' : 'text-gray-800'}`}>
        {title}
      </h3>
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-900">What kind of event are you hosting?</h2>
        <p className="text-gray-500 mt-2">This helps us tailor the experience for your needs.</p>
      </div>
